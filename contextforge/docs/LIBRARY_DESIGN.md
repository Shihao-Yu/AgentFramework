...
cf = ContextForge(framework_db_url="postgresql+asyncpg://localhost/mydb")

# Full configuration with Azure AD auth
cf = ContextForge(
    config=ContextForgeConfig(
        framework_db_url="postgresql+asyncpg://localhost/mydb",
        db_schema="agent",
...
class ContextForgeConfig(BaseSettings):
    ...
    CONTEXTFORGE_ prefix (e.g., CONTEXTFORGE_FRAMEWORK_DB_URL).
    """
    model_config = SettingsConfigDict(env_prefix="CONTEXTFORGE_")
    
    # Database
    framework_db_url: str
    db_schema: str = "agent"
    db_pool_size: int = 10
...
app = FastAPI()
cf = ContextForge(framework_db_url="postgresql+asyncpg://localhost/kb")
...
cf = ContextForge(
    framework_db_url="postgresql+asyncpg://localhost/test_db",
...
app = FastAPI()
cf = ContextForge(framework_db_url="...")
...
cf = ContextForge(
    config=ContextForgeConfig(
        framework_db_url=os.getenv("FRAMEWORK_DB_URL"),
    ),
    embedding_provider=OpenAIEmbeddingProvider(
...
cf = ContextForge(
    framework_db_url=get_secret("framework_db_url"),
)
```
